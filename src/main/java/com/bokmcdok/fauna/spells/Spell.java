package com.bokmcdok.fauna.spells;
...
/**
 * Base class for spells, effects that can be brought into the world via magic.
 */
public abstract class Spell {
     ...
     * @return The square of the spells range.
     */
     ...
     */
    protected RayTraceResult rayTrace(World world, double range, boolean checkEntityCollision, @Nonnull Entity shooter, RayTraceContext.BlockMode blockMode, Predicate<Entity> entityFilter) {
        Vector3d eyePosition = shooter.getEyePosition(1.0F);
        Vector3d lookDirection = shooter.getLook(1.0F).scale(range);
        Vector3d lookTarget = eyePosition.add(lookDirection);
        ...
        if (checkEntityCollision) {
            ...
            AxisAlignedBB boundingBox = new AxisAlignedBB(shooter.func_233580_cy_()).grow(range);
